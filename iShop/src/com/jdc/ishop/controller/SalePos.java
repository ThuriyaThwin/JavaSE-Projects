...
public class SalePos implements Initializable, PrintListener, Consumer<Item> {
	...
	private void search() {
		...
		itemBox.getChildren()
				.addAll(items.stream().map(a -> new ItemView(a, SalePos.this)).collect(Collectors.toList()));
	...
	private void initQueue() {
		...
		for (int i = 0; i < carts.size(); i++) {
			...
			lab.setOnMouseClicked(event -> {
				...
				cargo.getChildren().clear();
				ShoppingCart targetCart = carts.get(index);
				targetCart.setTotalValues();
				cargo.getChildren().add(targetCart);
				initQueue();
			});
